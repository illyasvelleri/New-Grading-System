...
export default async function handler(req, res) {
  ...
  const bulkOps = students.map((student) => {
    ...
    if (!student.monthlySummary) {
      student.monthlySummary = {};
    }

    const currentMonth = student.monthlySummary[month] || {};
    if (currentMonth[trimmed] === undefined) {
      currentMonth[trimmed] = 0;
      // Ensure student.monthlySummary[month] exists before assignment
      student.monthlySummary[month] = currentMonth;

      return {
        updateOne: {
          filter: { _id: student._id },
          update: { monthlySummary: student.monthlySummary }
        }
      };
    }
    return null;
